...
Everything here is a plain Emacs-lisp switch.  The defaults are derived
from `my-machine' below, so this file can be copied to any of my machines
unmodified; edit a `setq' here if a machine wants something different.

** Machine

Which machine this configuration is running on.  Used only to pick the
defaults for the switches below -- nothing else should branch on it.

Detected from `system-name' by prefix, because these hosts do not always
report a bare name: this Mac calls itself "calculon-3.local".  An
unrecognized host falls back to `generic', which is a plain GTK/X11 Linux
setup.  Set `my-machine' by hand if the guess is wrong.

#+BEGIN_SRC emacs-lisp
  (defvar my-machine
    (let ((host (downcase (car (split-string (system-name) "\\.")))))
      (cond ((string-prefix-p "bender"    host) 'bender)
            ((string-prefix-p "calculon"  host) 'calculon)
            ((string-prefix-p "hypnotoad" host) 'hypnotoad)
            ((string-prefix-p "zoidberg"  host) 'zoidberg)
            ((eq system-type 'cygwin)           'cygwin)
            ((eq system-type 'darwin)           'calculon)
            (t                                  'generic)))
    "Symbol naming the machine this Emacs is running on.
Only used to choose defaults in the Global toggles section.")
#+END_SRC

** Graphical backend library

Select athena/X11 (t) or GTK (nil) as the graphical display package.
Requires an Emacs actually built with the Athena toolkit; nil elsewhere.
...
Enable vterm terminal emulation.  Off on bender, which has no working
vterm module build.

#+BEGIN_SRC emacs-lisp
  (setq enable-vterm (not (eq my-machine 'bender)))
#+END_SRC

** Default font

The font used for graphical frames, as an X logical font description.
DejaVu Sans Mono everywhere except this Mac, at the size each display
wants.  See the Font section under Display for where this is applied.
...
Customized insertion of timestamp: insert it, then start a new line
with a trailing space ready for typing.

#+BEGIN_SRC emacs-lisp
(defun macrotimestamp ()
  "Insert a formatted timestamp, then a new line with a trailing space."
  (interactive)
  (insert-timestamp)
  (newline)
  (backward-char)
...
The font itself is chosen per machine by `my-default-font', in the Global
toggles section at the top of this file.
...
Hide files matching a regexp from the current Dired listing (mark
matching files, then remove those lines from the display; the files
themselves are untouched).  Use `M-x revert-buffer' to bring them back.

#+BEGIN_SRC emacs-lisp
  (defun omitmod ()
    "Hide .mod files from the current Dired listing."
    (interactive)
    (dired-mark-files-regexp "\\.mod$")
    (dired-do-kill-lines))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun omitdotfiles ()
    "Hide dotfiles from the current Dired listing."
    (interactive)
...
#+BEGIN_SRC emacs-lisp
  (defun omit-fortran ()
    "Hide dotfiles, .o, .mod, and backup (~) files from the current Dired listing."
    (interactive)
    (dired-mark-files-regexp "^\\.\\|\\.o$\\|\\.mod$\\|~$")
    (dired-do-kill-lines))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun quitnclean ()
  "Kill the running subjob (e.g. tail -f) and purge its output from the buffer."
  (interactive)
  (comint-quit-subjob)
...
NOTE: .pl files are assigned to cperl-mode (see Auto-assign modes,
above), so settings here use the cperl-* variables, not the perl-*
variables that belong to the older, separate perl-mode.
...
#+BEGIN_SRC emacs-lisp
  (defun clean-after-ediff ()
    "Restore a simple 2-window, side-by-side layout after an Ediff session."
    (interactive)
...
#+BEGIN_SRC emacs-lisp
  (defun fullcleanediff ()
    "Close all the extra windows opened by an Ediff session, leaving a single window."
    (interactive)
    (delete-other-windows))
...
** shell-mode (interactive M-x shell)

Define a hook that will start certain things when an interactive
shell (M-x shell) starts: The home key, cycling up & down through
command history.

NOTE: This is shell-mode (the comint-based interactive shell buffer),
not shell-script-mode/sh-mode (editing .sh files) below.
...
** shell-script-mode (editing scripts)
...
#+BEGIN_SRC emacs-lisp
  (unless enable-vterm
    (defun my-term-sentinel-kill-buffer (orig-fun proc msg)
      "Kill the term buffer once PROC has exited."
...
#+BEGIN_SRC emacs-lisp
  (unless enable-vterm
    ...
    (defun my-term-force-bash (&rest _)
      "Override `term''s interactive spec to always run `my-term-shell'."
      (interactive (list my-term-shell)))
    (advice-add 'term :before #'my-term-force-bash)
    )
#+END_SRC
...
This used to advise =vterm-sentinel=, copied from the ansi-term recipe at
http://emacs-journey.blogspot.com.au/2012/06/improving-ansi-vterm.html.
No such function exists -- vterm's is the private =vterm--sentinel= -- so
the advice never ran.  vterm does this itself via =vterm-kill-buffer-on-exit=,
which already defaults to t; set it explicitly so the intent is visible and
...
Unlike =term=, whose argument is the program to run, =vterm='s optional
argument is a buffer name -- so overriding its interactive spec with a
shell path named the buffer "/bin/bash" rather than choosing the shell.
vterm reads the shell from =vterm-shell=, so set that instead.

#+BEGIN_SRC emacs-lisp
  (when enable-vterm
    (defvar my-vterm-shell "/bin/bash")
    (setq vterm-shell my-vterm-shell)
  )
#+END_SRC
...
Start the Emacs server inside this (GUI) session so that =emacsclient=
can open files in it instead of launching a separate Emacs.  This is
what the =~/bin/ec= script and git's =core.editor= connect to.

Guarded with =server-running-p= so that starting a second Emacs (or a
=--daemon= session, which starts its own server) does not complain about
an existing socket.

Wrapped in =condition-case= because =server-start= signals an error when
the socket directory is considered unsafe -- e.g. =/tmp/emacs<uid>= owned
by another user, which happens on shared/multi-user machines such as HPC
login nodes.  Without this, such a machine would print an error on every
startup.  Failing to get a server is never worth breaking init over.
...
#+BEGIN_SRC emacs-lisp
  ...
  (unless (server-running-p)
    (condition-case err
        (server-start)
      (error
       (message "Emacs server not started: %s" (error-message-string err)))))
#+END_SRC
