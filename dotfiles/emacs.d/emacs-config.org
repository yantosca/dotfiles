...
#+BEGIN_SRC emacs-lisp
  (when enable-vterm
    (defun my-vterm-sentinel-kill-buffer (orig-fun proc msg)
      "Kill the vterm buffer once PROC has exited."
      (if (memq (process-status proc) '(signal exit))
          (let ((buffer (process-buffer proc)))
            (funcall orig-fun proc msg)
            (kill-buffer buffer))
        (funcall orig-fun proc msg)))
    (advice-add 'vterm-sentinel :around #'my-vterm-sentinel-kill-buffer)
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (when enable-vterm
    ...
    (defun my-vterm-force-bash (&rest _)
      "Override `vterm''s interactive spec to pass `my-vterm-shell'."
      (interactive (list my-vterm-shell)))
    (advice-add 'vterm :before #'my-vterm-force-bash)
  )
#+END_SRC
